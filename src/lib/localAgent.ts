import type { MessageMeta, ParsedExpense, PaymentMethod } from '@/types'
import { brl, paymentLabel } from '@/lib/format'
import { todayISO } from '@/lib/dates'

export type AgentReply = { reply: string; meta?: MessageMeta }
...
function parseInstallments(t: string): number {
  ...
  if (x && x[1]) return parseInt(x[1])
  const v = t.match(/(?:em|de)\s+(\d+)\s*(?:vezes|parcelas|x)/)
  if (v && v[1]) return parseInt(v[1])
  const p = t.match(/(\d+)\s*parcelas/)
  if (p && p[1]) return parseInt(p[1])
  return 1
}
