import type { ChatMessage, ChatRole, Expense, MessageMeta, NewExpense, PaymentMethod } from '@/types'
...
type Seed = [string, number, string, PaymentMethod, number, number, number]
const SEED: Seed[] = [
...
function readExpenses(): Expense[] {
  ...
  if (raw == null) {
    ...
    localStorage.setItem(K_EXP, JSON.stringify(seed))
    return seed
  }
  try {
...
function writeExpenses(rows: Expense[]) {
  localStorage.setItem(K_EXP, JSON.stringify(rows))
}

const byCreatedDesc = (a: Expense, b: Expense) => ((a.created_at || '') < (b.created_at || '') ? 1 : -1)
...
export const demoStore = {
  ...
  async last(): Promise<Expense | null> {
    ...
    return rows.slice().sort(byCreatedDesc)[0] ?? null
  },

  async update(id: string, patch: Partial<NewExpense>): Promise<void> {
    writeExpenses(readExpenses().map((r) => (r.id === id ? { ...r, ...patch } : r)))
  },

  async remove(id: string): Promise<void> {
    writeExpenses(readExpenses().filter((r) => r.id !== id))
  },

  async removeGroup(m: ExpenseGroupMatch): Promise<void> {
    writeExpenses(
      readExpenses().filter(
        (r) =>
          !(
  ...
  async chatInsert(role: ChatRole, content: string, meta?: MessageMeta): Promise<void> {
    ...
    localStorage.setItem(K_CHAT, JSON.stringify(list))
  },
